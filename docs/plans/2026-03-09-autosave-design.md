...
New subcommand `hyprflow autosave` that captures sessions with automatic rotation,
keeping only the last N autosave sessions. Manual sessions are never affected.

## Subcommand: `hyprflow autosave`
...
1. Capture session (same flow as `hyprflow save`)
...
Run 'hyprflow autosave --install' to set up the systemd timer.
```
...
To disable: hyprflow autosave --uninstall
```

### `--install` behavior

Generates two files in `~/.config/systemd/user/`:
- `hyprflow-autosave.service` — `ExecStart=hyprflow autosave --now`
- `hyprflow-autosave.timer` — `OnUnitActiveSec=10min`

Does NOT activate. Prints:
```
Created:
  ~/.config/systemd/user/hyprflow-autosave.service
  ~/.config/systemd/user/hyprflow-autosave.timer

To enable and start:
  systemctl --user enable --now hyprflow-autosave.timer

To check status:
  systemctl --user status hyprflow-autosave.timer
  journalctl --user -u hyprflow-autosave.service
```

### `--uninstall` behavior

1. `systemctl --user disable --now hyprflow-autosave.timer`
...
- Manual sessions (`hyprflow save work`) are never pruned by rotation
